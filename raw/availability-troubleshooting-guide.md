...
## Availability History and Auditing

The Availability System logs all changes impacting supply/demand calculations. Use this for compliance auditing, trend analysis, and troubleshooting.

### Available History Records

#### Table 50014 Extensions: CLE Item Avail. by Period (Historical)

When snapshots are enabled, the system stores timestamped copies of availability calculations:

- "Calculation Timestamp": When calculation was performed
- "Snapshot Date": Which date's availability was calculated
- "Supply", "Demand", "Net", "Running Total": Values from that calculation
- "Changed By": User who triggered calculation
- "Change Reason": Reason code (Order Insert, SO Ship, PO Receipt, etc.)

### Auditing Availability Changes

**Question: Why did availability drop from 100 to 80?**

**Process:**

1. **View Availability History** (Page 50XXX)
   - Filter Item No., Location
   - Review "Calculation Timestamp" column
   - Find calculations before/after the drop

2. **Review Changes**
   - Click on timestamp
   - Page shows:
     - Old Supply / Old Demand / Old Net
     - New Supply / New Demand / New Net
     - What changed (e.g., SO100 shipped 20 units → Demand -20)

3. **Drill to Source Document**
   - Click "Change Reason" link
   - Goes directly to sales order, PO, or production document
...
1. **Open Availability Trends** (Page 50XXX)
2. Select item + location
3. Choose date range (e.g., last 2 years)
4. System charts:
   - Availability over time (line)
   - Supply volume (green bars)
   - Demand volume (red bars)
   - Seasonality peaks

**Using trends for planning:**

- Identify lowest-availability months (typically when to order ahead)
- See which periods need larger safety buffers
- Compare year-over-year to spot improvements/degradations

### Compliance Reporting

**Availability Variance Report** (Page 50XXX)

Shows all instances where:
- Promised availability exceeded actual (over-promised)
- Demand forecasts missed by >15%
- Safety stock calculations out of range
- Reserves held longer than necessary

**To generate:**

1. **Reports > Availability Variance Report**
2. Set date range
3. Optional: Filter item family, location, salesperson
...
|-------|----------------|
| "Over-Promised Days" | Days availability calculation was inaccurate |
| "Quantity Variance" | Difference between promised and actual |
| "Contributing Factor" | Why variance occurred (Late PO, Early SO, etc.) |
| "Assigned To" | Responsible party (salesperson, buyer, planner) |

### Troubleshooting with History

**Scenario: User claims availability was wrong 3 weeks ago**

1. Note the exact date/time
2. **Page 50XXX Availability History**
3. Filter to that date, go to timestamp nearest the claim
4. Review what supply/demand was used that day
5. Verify if data has since been corrected
6. Determine if availability shown was accurate given information available then

## Performance Tuning

As item catalogs grow (1000+s of items), availability calculations can slow. Use these techniques to optimize.
...
2. "Active Locations for Availability": Mark only active warehouse locations
3. Disable "CLE Hide from Availability" = FALSE on dormant locations
...
2. Set "Forecast Period" = "6 Months" (default already)
3. Use date range filters to show only next 3 months
4. For long-range planning, use "Summary" view (fewer details, faster)

**Expected improvement:** 40-60% faster.

### Optimization 3: Production Order Filtering

**Issue:** Including all production statuses (Planned, Released, Finished) creates detail.
...
**Trade-off:** Availability up to 24 hours old during day (acceptable for most planning).
