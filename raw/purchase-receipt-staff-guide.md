# Purchase Receipt Process - Staff Guide
...
3. [Standard Receipt Process](#standard-receipt-process)
4. [Quality Hold Process](#quality-hold-process)
...
### What is the Purchase Receipt Process?

The Purchase Receipt system allows you to receive incoming shipments from vendors into the warehouse inventory system. When a vendor delivery arrives, you use this system to:

- Record what was received and in what quantity
- Verify items against the purchase order
- Route damaged or questionable items to quality hold
- Update inventory automatically

### Why It Matters

Accurate receiving is critical because:
- **Inventory accuracy** - system knows what's actually in the warehouse
- **Vendor accountability** - tracks what vendors shipped vs. ordered
- **Quality control** - separates problem items for inspection
- **Order fulfillment** - items become available for customer orders
...
**Navigation:** Search for "Purchase Receipts" or use the menu path:
...
| **Unit of Measure** | Each, case, pallet, etc. | No |
| **Qty. per Unit of Measure** | How many base units per UoM | No |
| **Outstanding Quantity** | What's still expected | No |
| **Qty. to Receive** | What you're receiving now | **Yes** |
| **Quantity Received** | Previously received | No |
| **Location Code** | Warehouse location | No |
| **Bin Code** | Specific bin | **Yes** |
...
- Outstanding Quantity: 1000
- Count: 1000
- Action: Leave Qty. to Receive at 1000

**Scenario B: Received Less Than Expected**
- Outstanding Quantity: 1000
- Count: 850
- Action: Change Qty. to Receive to 850
- Note: Remaining 150 stays on PO for next shipment

**Scenario C: Received More Than Expected**
- Outstanding Quantity: 1000
- Count: 1050
- Action: **STOP** - Contact supervisor before proceeding
- Reason: May need PO adjustment or return
...
- If variant doesn't exist: Contact supervisor
...
- If you try to post without a variant, the system shows: **"You must select a variant"**
- Action: Select the variant that matches the actual blooming stage of the received plants
- If unsure which stage the plants are in, ask your supervisor or the grower team

#### Step 5: Verify Bin Codes (If Applicable)

Some warehouses use directed put-away:

1. Check **Bin Code** field on each line
2. If blank or wrong location, update it
3. Match to where you're physically putting the items

**Example:**
- Item: Pot-4inch
- Bin Code: `RECV-01` (default receiving bin)
- Change to: `A-12-03` (where you're storing it)

#### Step 6: Post the Receipt (F9)
...
**Example After Split:**
- Line 10000: Item 10400, Qty 850, Location: MAIN, Bin: A-12
...
### Problem: Wrong Variant Not in Dropdown

**Cause:** Variant hasn't been created in system

**Solution:**
1. STOP - do not create variants yourself
2. Contact supervisor or inventory coordinator
3. Options:
   - Supervisor creates variant, then you can proceed
   - Reject shipment back to vendor
   - Put entire line on hold while resolving
...
### Problem: Bin Code Not Accepting Value

**Cause:** Bin doesn't exist in that location or bin management not enabled

**Solution:**
1. Use the dropdown to select valid bin
2. If bin not in list, leave blank or use default receiving bin
3. Contact warehouse manager if new bin needed

---

### Problem: Posting Takes Long Time or Freezes

**Cause:** Large number of lines or system load

**Solution:**
1. Wait 30-60 seconds - posting can be slow
2. Do NOT press Post multiple times
3. If truly frozen (>2 minutes):
   - Note what you were doing
   - Close the page (don't save)
   - Contact IT support
   - Try again after they review
...
□ Update Bin Codes if applicable
□ Press F9 to post
□ Confirm posting
□ Done!
```
...
**Header:**
- **No.** → Purchase order number (read-only)
- **Vendor Shipment No.** → REQUIRED - vendor packing slip number
- **Posting Date** → When receipt is recorded (default: today)

**Lines:**
- **Qty. to Receive** → What you're receiving NOW
- **Outstanding Quantity** → What's still expected
- **Variant Code** → Can change if vendor sent different variant
- **Bin Code** → Where you're putting items
...
- Contact on-call manager (see posted contact list)

**Non-Critical Issues:**
- Note the issue
- Complete what you can
- Report at start of next business day

---

## Related Processes

- **Purchase Order Creation** - Done by purchasing team
- **Quality Inspection** - Done by quality team on hold items
- **Inventory Put-Away** - Moving from receiving to storage bins
- **Vendor Invoice Matching** - Accounting reconciles receipt to invoice
