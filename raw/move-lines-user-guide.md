...
The **Move Lines** feature allows you to transfer sales order lines (or portions of them) from one sales order to another. This is essential for:

- Consolidating multiple small orders into one shipment
- Splitting orders based on availability or delivery schedules
- Adjusting shipment dates for specific items
- Reorganizing orders when customer requirements change

Move Lines integrates with the availability system, blanket orders, and fee calculations to ensure accurate order management.
...
- New shipment dates are valid (not in the past, respects business rules)

---

## Opening Move Lines

### From Sales Order Card/List
...
4. **Enter Qty. to Move**:
   ...
   - Can enter more if blanket order supports it (see Blanket Order section)
...
### Problem: "Availability is not sufficient on that date" Error
...
1. Check availability forecast to understand supply/demand
2. Verify blanket order quantities and remaining balance
3. Contact your supervisor for business rule questions
4. Contact IT support for system errors or technical issues
5. Reference the IT Troubleshooting Guide for technical details
