---
title: Availability System — IT Troubleshooting Guide
type: howto
tags: [availability, it-support, troubleshooting, system-architecture, calculation-algorithm]
created: 2026-04-21
updated: 2026-04-21
sources: [troubleshooting-guide.md]
---

# Availability System — IT Troubleshooting Guide

Technical architecture, configuration, and troubleshooting for the real-time item availability calculation system.
...
- **Recently fixed**: Production component quantity display (Feb 2026)
...
```
// WRONG (old code)
TempSalesLine.Quantity := (ProdOrderComponent."Quantity (Base)" / 
  Item."CLE Qty. per Sales UOM") * (-1)

// CORRECT (fixed)
TempSalesLine.Quantity := (ProdOrderComponent."Remaining Quantity" / 
  Item."CLE Qty. per Sales UOM") * (-1)
```
...
- Set breakpoint in Codeunit 50064, line 13-53
- Check LocationFilter value
- Check SupplyDates collection
- Check TempAvailLine records after CreateTempLines()
...
Should return active locations only (those without "CLE Hide from Availability" = true):
```
Example: "EVANSTON|GRAYSLAKE|..." (active locations only)
```
...
```
Queries should use LocationFilter variable for all tables
...
```
ProdOrderComponent.SetRange("Status", 
...
- `CLE Qty. per Sales UOM` — FlowField calculating Sales UoM conversion factor
...
- Check: Date filter syntax and matching date fields
